...
**Status:** under implementation. S1a, S1b, S2, S3 and S5 are merged in code, S5's testbed run has happened and its fixes are merged, S4 is gated and S6 waits. The Status column of the §6 table is kept current; the design text is not rewritten after implementation (dated amendments at the end).

- **S1a** implemented and merged 2026-09-14 (tdns #650 → 2af063dc, tdns-mp #66 → f11db7a).
- **S1b** implemented and merged 2026-09-14 (tdns #656 → b37e32ab, tdns-mp #67 → 5f8f906), as Amendment 1 describes it.
- **S2** implemented and merged 2026-09-15 (tdns #659 → 49df2c3a); nothing registers an owner yet, and tdns-mp re-pins with S3.
- **S3** implemented and merged 2026-09-16 (tdns #664 → 155ea6b7, tdns-mp #76 → 0dffb47): tdns-mp's state machine per `docs/2026-09-15-mp-key-lifecycle-transition-table.md` (kept current with the code; r12), rolled out per zone through `key-lifecycle-zones`. The first lab run of S3 and S5's first half (2026-09-16, one cell owned: the take, a ZSK roll and a KSK roll through the machine) gave five findings, fixed and merged 2026-09-17 (tdns-mp #82 → 86b3eed).
- **S5, first half** implemented and merged 2026-09-16 (tdns #671 → a09e5cde, tdns-mp #80 → 5b56aa4): the part that waits for no open question (arrow 1's CDS from the DS set, the inventory's columns and the agent's DS intent, the combiner's CDS off, the signer running the DS engine). Q2 and Q9 were decided on 2026-09-17, both as recommended.
- **S5, second half, part one** implemented and merged 2026-09-17 (tdns #696 → 9ce5f970, tdns-mp #83 → fbf4e09): provider and state on the DNSKEY distribution (#58) and `ds` on the other providers' keys from it.
- **S5, second half, part two** implemented and merged 2026-09-17 (tdns-mp #86 → 4525427): the leader's delegation sync follows a change of the DS set on its own, with the hand-over at a leader change (T5.4, T5.8).
- **S5 on the testbed** (2026-09-17/18), with every signing provider on these releases:
  - Found while preparing the run: the signer started no resolver, so nothing observed the parent's DS for the machine and a retired KSK of an owned zone never left `retired`; a signer that owns a zone now starts one (tdns-mp #87, the transition table r11).
  - Found on the run: in the signer the DS engine's request queue did not exist (tdns makes it only when the KeyDB exists at its MainInit; tdns-mp builds the KeyDB after), so tdns's `KeysChanged` returned without waking the engine and an owned zone's CDS followed its DS set only at the next transfer from the combiner, not when a `ds` column changed; the signer now makes the queue before any engine starts (tdns-mp #87).
  - Seen the same day: three signers serving one CDS set with a KSK of each provider, the other providers' from their `ds` word (arrow 1 with foreign KSKs).
...
- **Open beside the steps:** per-zone standby counts (a tdns policy field), an algorithm rollover by the machine (table §6 O4), and a hand-over field naming the providers that have spoken (table §3).
