...
| S5 | tdns-mp | Provider and state on DNSKEY distribution, and foreign rows with both (#58). The DS set (D4). Arrow 1: the signer's CDS from `ds` rows, restored by `CollectDynamicRRs` after every transfer; the combiner stops synthesizing CDS. Arrow 2: inventory flags pushed on every flag change, the DS-intent provider on the agent, the leader gate on tdns's syncher. Delegation-sync setup that runs for the multi-provider agent. | Multi-provider DS towards the parent works as D4 says | **First half implemented, merged** 2026-09-16: tdns #671 → a09e5cde, tdns-mp #80 → 5b56aa4 (arrow 1: the signer serves the CDS of the owner's DS set, restored over transfers and followed by the DS engine, which tdns-mpsigner now runs; the inventory carries pub, sign, ds and Owned; the agent's DS intent from it; no combiner CDS; the mp agent's app type registered; setup and the DNSKEY-derived CDS gated). **Second half, part one, implemented** 2026-09-17 (tdns #696, tdns-mp #83; Q2 and Q9 decided that day): provider and state on the DNSKEY distribution, `ds` on the foreign rows from it (P3, E10), so D4 holds across peers on arrow 1 once every signing provider runs it. **Second half, part two, implemented** 2026-09-17 (tdns-mp `feature/s5-leader-ds-sync`): the leader's delegation sync follows a change of the DS set on its own (the agent asks for an explicit sync when the owner's DS intent, known, is not what it was; T5.4's leader half), a peer that does not lead asks for nothing, and a newly elected leader asks once, so a hand-over neither loses an update nor repeats one (T5.8; the explicit sync sends only a difference). **Left:** the same against a parent on the testbed, with the cutover watch (T5.5). |
