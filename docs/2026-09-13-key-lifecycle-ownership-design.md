...
- **What asks for the sync (r14).** The syncher compares the parent's DS with the DS intent and sends a difference, but only when asked, and a `ds` flip changes no DNSKEY and no delegation data, so nothing asked. The agent now asks: when an inventory leaves the owner's DS intent known and not what it was, the elected leader queues an explicit delegation sync; a peer that does not lead queues nothing, since the leader's own signer gets the same word (its own keys from its rows, the other providers' through #58). A newly elected leader asks once as well. The explicit sync analyses first and sends only a difference, so asking twice sends once, and a hand-over neither loses an update nor repeats one (T5.8). Both askers go through one gate: the zone syncs with its parent through its agents (parentsync=agent) and this agent leads. Only an inventory from a signer that runs the zone's key lifecycle counts, since the syncher's DS intent is the owner's for such a zone alone; the first one after a take therefore asks even with every key as it was. A request that finds the syncher's queue full is asked again shortly, a bounded number of times.
