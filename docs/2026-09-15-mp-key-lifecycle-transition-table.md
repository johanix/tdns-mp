...
**Status:** implemented, and kept current with the code (r12): S3's state machine as merged in tdns-mp #76, with the lab findings of #82 (r8), the foreign rows of #83 (r9) and what the leader is asked of #86. It began as the spec of S3's state machine, written before its code, as §3.4's table was for S1b (design `docs/2026-09-13-key-lifecycle-ownership-design.md`, test plan `docs/2026-09-14-key-lifecycle-ownership-test-plan.md` T3.1).
...
| r12 | 2026-09-21 | Open item O7, from the testbed's KSK roll: T9 for a SEP key must wait for the parent's DS. No rule changed. |
...
| O7 | A KSK rollover by the machine: T9 promotes the standby key on E6 with no look at the parent, and T10 retires the old key in the same step, so the old signatures go before the parent serves the new DS. On the testbed (2026-09-18) the parent refused the DS change every time: the rolled provider's DNSKEY RRset validated against no DS the parent held, and an unvalidated answer cannot change a DS. The conservative order (RFC 6781 §4.1.2): standby, the parent serves the new key's DS, the DS TTL passes, then promote and retire. | an event "DS present" (the parent serves the key's DS: E7's inverse, from the same resolver), and T9's guard for a SEP key requires it plus the parent's DS TTL, whatever asked for the promotion; a ZSK's T9 is unchanged; against a parent that serves no DS at all (O2's case) the guard is met at once |
