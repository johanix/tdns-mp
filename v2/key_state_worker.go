...
func transitionPublishedToStandby(conf *tdns.Config, kdb *tdns.KeyDB, now time.Time, propagationDelay time.Duration) {
	keys, err := kdb.GetDnssecKeysByState("", tdns.DnskeyStatePublished)
	if err != nil {
	...
	for _, key := range keys {
		...
		if err := kdb.UpdateDnssecKeyState(key.ZoneName, key.KeyTag, tdns.DnskeyStateStandby); err != nil {
...
func transitionRetiredToRemoved(conf *tdns.Config, kdb *tdns.KeyDB, now time.Time, propagationDelay time.Duration) {
	keys, err := kdb.GetDnssecKeysByState("", tdns.DnskeyStateRetired)
	if err != nil {
...
func maintainStandbyKeysForType(kdb *tdns.KeyDB, zoneName string, alg uint8, keytype string, expectedFlags uint16, isMP bool, standbyKeyCount int) {
	...
	standbyKeys, err := kdb.GetDnssecKeysByState(zoneName, tdns.DnskeyStateStandby)
	if err != nil {
	...
	publishedKeys, _ := kdb.GetDnssecKeysByState(zoneName, tdns.DnskeyStatePublished)
	publishedCount := countKeysByFlags(publishedKeys, expectedFlags)

	mpdistKeys, _ := kdb.GetDnssecKeysByState(zoneName, tdns.DnskeyStateMpdist)
...
func countKeysByFlags(keys []tdns.DnssecKeyWithTimestamps, expectedFlags uint16) int {
	count := 0
	for _, k := range keys {
