...
func (conf *Config) HsyncEngine(ctx context.Context, msgQs *MsgQs) {
	...
	for {
		...
		case req := <-conf.InternalMp.SyncStatusQ:
...
func (ar *AgentRegistry) SyncRequestHandler(ourId AgentId, req SyncRequest, synchedDataUpdateQ chan *SynchedDataUpdate, msgQs *MsgQs) {
	...
	case "SYNC-DNSKEY-RRSET":
		...
		if zd, exists := tdns.Zones.Get(string(req.ZoneName)); exists && zd.MP.MPdata != nil && !zd.MP.MPdata.ZoneSigned {
