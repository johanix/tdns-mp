...
usage() { echo "usage: sh run.sh start|stop|restart|status|converge|verify|scenario <name>|clean|redirect [--do]" >&2; exit 2; }
...
# port $world_dns. One privileged step, once per boot: a packet redirect.
redirect() {
	need_seeded
	wp=$(port world dns)
	case "$(uname -s)" in
	Darwin)
		...
		echo "macOS: pf redirect of 127.0.0.1:53 to the world server ($wp):"
		echo "  echo '$rule' | sudo pfctl -a mp-policy-matrix -f -"
		echo "  sudo pfctl -e"
		echo "undo: sudo pfctl -a mp-policy-matrix -F all"
		if [ "$1" = "--do" ]; then
			echo "$rule" | sudo pfctl -a mp-policy-matrix -f - && sudo pfctl -e 2>/dev/null; true
		fi ;;
	Linux)
		...
		echo "undo: the same lines with -D instead of -A"
		if [ "$1" = "--do" ]; then
			for proto in udp tcp; do
				sudo iptables -t nat -A OUTPUT -o lo -p "$proto" -d 127.0.0.1 --dport 53 -j REDIRECT --to-ports "$wp"
			done
		fi ;;
...
start() {
	...
	else echo "WARNING: 127.0.0.1:53 does not reach the world server -- agents cannot discover each other. Run: sh $SRC/run.sh redirect"; fi
}
...
status() {
	...
	echo
	if port53_ok; then echo "port 53 -> world: ok"; else echo "port 53 -> world: NOT reachable (run: sh run.sh redirect)"; fi
	echo
...
case "${1:-status}" in
	...
	clean)    stop 2>/dev/null; rm -rf "$RIG"; echo "removed $RIG" ;;
